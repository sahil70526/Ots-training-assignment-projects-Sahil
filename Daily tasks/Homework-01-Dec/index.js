...
myCar = new Car("BMW");
console.log(
  `Hi I am created with the help of class and costructer function \n ${myCar.carName}`
);
...
let fun1 = function (country) {
  return `${this.name} from  ${
    this.city
  } currentlly  ${this.presentGoal()}  and from country ${country}`;
};
...
// ------------------ practice for thrittling in real time function -------------------------

let count1 = 0;
const getdata1 = function () {
  count1++;
  console.log(`function is called after ${count1} time period`);
};
const throttle = function (fn, delay) {
  ...
  return function () {
    ...
    if (time - timer1 > delay) {
      timer1 = time;
      let obj6 = this;
      return fn.apply(obj6);
    }
  };
};

const throttlefn = throttle(getdata1, 1000);
